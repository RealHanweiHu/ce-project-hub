import { describe, it, expect } from "vitest";
import { PROJECT_CATEGORIES } from "./sop-templates";
import { PROJECT_MEMBER_ROLES } from "../drizzle/schema";

/**
 * P1-7：pe/mfg/sales 在全部 6 类模板里 visibleRoles 曾全为空——这三个角色进项目
 * 只看得到 Gate 任务；cert/battery_safety 在 ECO/IDR/JDM/OBT 也曾为空（换电芯二供的
 * ECO 反而没有电池安全参与）。这些都是跨轨都存在的职能，每类模板至少要有其可见任务
 * （具体产品无该职能时由裁剪移除，而不是模板里根本看不到）。
 */
const REQUIRED = ["pe", "mfg", "sales", "cert", "battery_safety"] as const;
const PROJECT_MEMBER_ROLE_SET = new Set<string>(PROJECT_MEMBER_ROLES);

describe("角色任务可见性覆盖", () => {
  for (const cat of PROJECT_CATEGORIES) {
    it(`${cat.id}：pe/mfg/sales/cert/battery_safety 各至少有一个可见任务`, () => {
      const present = new Set<string>();
      for (const p of cat.phases) for (const t of p.tasks) for (const r of t.visibleRoles ?? []) present.add(r);
      const missing = REQUIRED.filter((r) => !present.has(r));
      expect(missing, `${cat.id} 缺角色可见任务: ${missing.join(", ")}`).toEqual([]);
    });

    it(`${cat.id}：每个非 Gate 任务都有明确的项目内责任角色`, () => {
      const missing: string[] = [];
      const invalid: string[] = [];

      for (const phase of cat.phases) {
        for (const task of phase.tasks) {
          const roles = task.visibleRoles ?? [];
          for (const role of roles) {
            if (!PROJECT_MEMBER_ROLE_SET.has(role)) invalid.push(`${phase.id}/${task.id}:${role}`);
          }
          if (task.id !== phase.gateTaskId && roles.length === 0) missing.push(`${phase.id}/${task.id}`);
        }
      }

      expect(invalid, `${cat.id} 有非项目成员角色: ${invalid.join(", ")}`).toEqual([]);
      expect(missing, `${cat.id} 非 Gate 任务缺责任角色: ${missing.join(", ")}`).toEqual([]);
    });
  }
});
