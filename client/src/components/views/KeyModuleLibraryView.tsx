...
export function KeyModuleLibraryView() {
  ...
  return <div className="flex flex-col">
    ...
    <KeyModuleDetailDialog open={Boolean(selectedId)} onOpenChange={open => { if (!open) setSelectedId(null); }} detail={detail.data as KeyModuleBundle | undefined} loading={detail.isLoading} canApprove={canApprove} pending={pending}
      onEdit={() => { setEditing(detail.data as KeyModuleBundle); setEditorOpen(true); }} onConfirm={() => selectedId && confirm.mutate({ id: selectedId })} onApprove={() => selectedId && approve.mutate({ id: selectedId })} onReturn={() => selectedId && returnToDraft.mutate({ id: selectedId })}
