...
export function MetricsView({ project }: { project: Project }) {
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="ce-panel h-80 animate-pulse bg-stone-100/60" />
          ))}
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="p-6 space-y-4">
        <MetricsHeader mode={mode} onModeChange={setMode} customFrom={customFrom} customTo={customTo} onCustomFrom={setCustomFrom} onCustomTo={setCustomTo} />
        <div className="ce-panel p-8 text-center">
          <AlertTriangle size={24} className="mx-auto text-rose-500 mb-3" />
          <div className="text-sm text-stone-700">度量数据加载失败</div>
          <div className="text-xs text-stone-400 mt-1">{error.message}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <MetricTile icon={<AlertTriangle size={15} />} label="逾期率" value={formatPct(metrics.efficiency.overdueRatePct)} accent={metrics.efficiency.overdueRatePct ? 'text-rose-600' : undefined} />
        <MetricTile icon={<Bug size={15} />} label="缺陷 DI" value={formatNumber(metrics.quality.diValue)} accent={metrics.quality.diValue > 0 ? 'text-rose-600' : undefined} />
        <MetricTile icon={<CheckCircle2 size={15} />} label="Gate 一次通过" value={formatPct(metrics.process.gateFirstPassRatePct)} accent="text-emerald-700" />
      </div>
          ...
          <ChartFrame empty={!hasAnyValue(throughputData, ['count'])}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={throughputData} margin={{ top: 8, right: 8, left: -24, bottom: 0 }}>
                ...
                <Bar dataKey="count" name="完成任务" fill="#0f766e" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </ChartFrame>
        </MetricPanel>
            ...
            <ChartFrame empty={!hasAnyValue(qualityTrendData, ['opened', 'closed', 'cumulativeOpen'])}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={qualityTrendData} margin={{ top: 8, right: 12, left: -24, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="opened" name="新增" stroke="#dc2626" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="closed" name="关闭" stroke="#16a34a" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="cumulativeOpen" name="累计未关" stroke="#57534e" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </ChartFrame>
              ...
              <ChartFrame empty={!hasAnyValue(taskBurndownData, ['remaining', 'ideal'])}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={taskBurndownData} margin={{ top: 8, right: 12, left: -24, bottom: 0 }}>
                    ...
                    <Tooltip {...tooltipProps} />
                    <Legend wrapperStyle={{ fontSize: 11 }} />
                    <Line type="monotone" dataKey="remaining" name="实际剩余" stroke="#0f766e" strokeWidth={2} dot={false} />
                    <Line type="monotone" dataKey="ideal" name="理想线" stroke="#f59e0b" strokeWidth={2} strokeDasharray="4 4" dot={false} connectNulls />
                  </LineChart>
                </ResponsiveContainer>
              </ChartFrame>
            </ChartBlock>
              ...
              <ChartFrame empty={!hasAnyValue(defectBurndownData, ['remaining'])}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={defectBurndownData} margin={{ top: 8, right: 12, left: -24, bottom: 0 }}>
                    ...
                    <YAxis tick={axisTick} axisLine={false} tickLine={false} allowDecimals={false} />
                    <Tooltip {...tooltipProps} />
                    <Line type="monotone" dataKey="remaining" name="未关闭缺陷" stroke="#dc2626" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </ChartFrame>
            </ChartBlock>
          ...
          <ChartFrame empty={!hasAnyValue(phaseDurationData, ['plannedDays', 'actualDays'])}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={phaseDurationData} margin={{ top: 8, right: 8, left: -24, bottom: 0 }}>
                ...
                <Tooltip {...tooltipProps} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar dataKey="plannedDays" name="计划天数" fill="#78716c" radius={[3, 3, 0, 0]} />
                <Bar dataKey="actualDays" name="实际天数" fill="#2563eb" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </ChartFrame>
        </MetricPanel>
...
function MetricsHeader({
  ...
  return (
    <div className="ce-panel px-4 py-3 flex flex-col lg:flex-row lg:items-center gap-3">
      <div className="flex items-center gap-2 min-w-0">
        <Gauge size={17} className="text-stone-600 shrink-0" />
        <div className="min-w-0">
          <h2 className="font-serif text-lg text-stone-900 leading-tight">项目度量</h2>
          <p className="text-[10px] font-mono uppercase tracking-widest text-stone-400">PROJECT METRICS</p>
        </div>
      </div>
        ...
        {mode === 'custom' && (
          <div className="flex items-center gap-2">
            <input
              ...
              className="ce-control px-2 py-1.5 text-xs font-mono text-stone-700 w-36"
            />
...
function WindowButton({ active, icon, label, onClick }: { active: boolean; icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-mono uppercase tracking-wider transition-colors ${
        active ? 'bg-stone-900 text-stone-50' : 'text-stone-500 hover:bg-stone-50'
      }`}
    >
      {icon}
      {label}
...
function MetricPanel({ title, kicker, icon, children }: { title: string; kicker: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="ce-panel p-4">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-8 h-8 border border-stone-200 bg-stone-50 flex items-center justify-center text-stone-600 shrink-0">
          {icon}
        </div>
        <div>
          <h3 className="font-serif text-base text-stone-900 leading-tight">{title}</h3>
          <p className="text-[10px] font-mono uppercase tracking-widest text-stone-400">{kicker}</p>
        </div>
      </div>
      {children}
    </section>
...
function InlineStat({ label, value, tone = 'stone' }: { label: string; value: string | number; tone?: 'stone' | 'rose' | 'emerald' }) {
  const toneClass = tone === 'rose' ? 'text-rose-600' : tone === 'emerald' ? 'text-emerald-700' : 'text-stone-900';
  return (
    <div className="border border-stone-200 bg-stone-50/70 px-3 py-2 min-w-0">
      <div className="text-[10px] font-mono uppercase tracking-wider text-stone-400 truncate">{label}</div>
      <div className={`mt-0.5 text-xl font-serif font-semibold ${toneClass}`}>{value}</div>
    </div>
  );
}
...
function ChartBlock({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="text-[10px] font-mono uppercase tracking-widest text-stone-400 mb-2">{title}</div>
      {children}
    </div>
  );
}
...
function ChartFrame({ empty, children }: { empty: boolean; children: React.ReactNode }) {
  return (
    <div className="h-[240px] min-h-[240px]">
      {empty ? (
        <div className="h-full border border-dashed border-stone-200 bg-stone-50/60 flex items-center justify-center text-xs font-mono text-stone-400">
          暂无足够数据
        </div>
      ) : children}
    </div>
      ...
      <ChartFrame empty={!hasAnyValue(data, ['count'])}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ top: 4, right: 12, left: 8, bottom: 4 }}>
            ...
            <Tooltip {...tooltipProps} />
            <Bar dataKey="count" name="数量" fill="#7c3aed" radius={[0, 3, 3, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </ChartFrame>
    </div>
