...
export function ProductLifecycleGovernancePanel({ product }: { product: GovernanceProduct }) {
  ...
  const { users: userOptions, nameById: userById } = useUserNames();
        ...
        {canMaintain && product.lifecycleState !== 'eol' && (
          <div className="rounded-[10px] border border-border bg-secondary p-4 space-y-3">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <select value={software.qaOwnerUserId ?? ''} onChange={(e) => setSoftware({ ...software, qaOwnerUserId: e.target.value ? Number(e.target.value) : null })} className="rounded-[7px] border border-border bg-card px-3 py-2 text-sm"><option value="">独立验证责任人</option>{userOptions.map((item) => <option key={item.id} value={item.id}>{userById.get(item.id)}</option>)}</select>
        ...
        {(eolCoreEditable || eolPlan.data) && (
          <div className="rounded-[10px] border border-border bg-secondary p-4 space-y-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <select disabled={!eolCoreEditable} value={eol.ownerUserId ?? ''} onChange={(event) => setEol({ ...eol, ownerUserId: event.target.value ? Number(event.target.value) : null })} className="rounded-[7px] border border-border bg-card px-3 py-2 text-sm"><option value="">EOL 责任人</option>{userOptions.map((item) => <option key={item.id} value={item.id}>{userById.get(item.id)}</option>)}</select>
              <select disabled={!eolCoreEditable} value={eol.approverUserId ?? ''} onChange={(event) => setEol({ ...eol, approverUserId: event.target.value ? Number(event.target.value) : null })} className="rounded-[7px] border border-border bg-card px-3 py-2 text-sm"><option value="">独立审批人</option>{userOptions.map((item) => <option key={item.id} value={item.id}>{userById.get(item.id)}</option>)}</select>
            </div>
