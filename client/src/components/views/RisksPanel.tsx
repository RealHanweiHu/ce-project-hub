...
const SEVERITY_CONFIG: Record<RiskSeverity, { label: string; className: string }> = {
  ...
  low: { label: '低', className: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
};

const STATUS_CONFIG: Record<RiskStatus, { label: string; className: string }> = {
  open: { label: '识别', className: 'bg-rose-50 text-rose-700 border-rose-200' },
  mitigating: { label: '缓解中', className: 'bg-amber-50 text-amber-700 border-amber-200' },
  watching: { label: '观察中', className: 'bg-blue-50 text-blue-700 border-blue-200' },
  closed: { label: '已关闭', className: 'bg-stone-100 text-stone-500 border-stone-200' },
};
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <RiskMetric label="高风险" value={summary.high} accent={summary.high > 0 ? 'text-rose-600' : undefined} />
        <RiskMetric label="中风险" value={summary.medium} accent={summary.medium > 0 ? 'text-amber-600' : undefined} />
        ...
        <div className="ce-panel p-4 space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            {editingId && (
              <button
                type="button"
                onClick={cancelEdit}
                className="inline-flex h-8 w-8 items-center justify-center border border-stone-300 text-stone-500 hover:bg-stone-50"
                title="取消编辑"
              >
                <X size={14} />
              </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
            <input
              ...
              placeholder="风险标题"
              className="lg:col-span-4 border border-stone-200 px-3 py-2 text-sm outline-none focus:border-stone-400"
            />
            <select
              value={form.severity}
              onChange={(event) => setForm((prev) => ({ ...prev, severity: event.target.value as RiskSeverity }))}
              className="lg:col-span-2 border border-stone-200 px-3 py-2 text-sm outline-none focus:border-stone-400"
            >
          ...
          <div className="flex justify-end">
            <button
              ...
              onClick={submit}
              disabled={isSaving}
              className="inline-flex items-center gap-1.5 px-3 py-2 text-xs bg-stone-900 text-white hover:bg-stone-700 disabled:opacity-50"
            >
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <div className="ce-panel p-5 text-sm text-stone-400">暂无风险项</div>
        ) : (
          risks.map((risk) => {
            ...
            return (
              <div key={risk.id} className={`ce-card p-4 ${closed ? 'opacity-70' : ''}`}>
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                  <div className="min-w-0 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 border text-[10px] font-mono ${severity.className}`}>
                        <AlertTriangle size={11} />{severity.label}
                      </span>
                      <span className={`px-1.5 py-0.5 border text-[10px] font-mono ${status.className}`}>{status.label}</span>
                      {risk.owner && <span className="text-[11px] text-stone-400">负责人：{risk.owner}</span>}
                      {risk.targetDate && <span className="text-[11px] font-mono text-stone-400">目标：{risk.targetDate}</span>}
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-stone-900 break-words">{risk.title}</h4>
                      {risk.description && <p className="mt-1 text-xs text-stone-500 whitespace-pre-wrap leading-relaxed">{risk.description}</p>}
                    </div>
                    {(risk.mitigationPlan || risk.contingencyPlan) && (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 text-xs">
                        {risk.mitigationPlan && (
                          <div className="bg-stone-50 border border-stone-200 p-2">
                            <div className="text-[10px] font-mono text-stone-400 mb-1">缓解措施</div>
                            <div className="text-stone-700 whitespace-pre-wrap">{risk.mitigationPlan}</div>
                          </div>
                        )}
                        {risk.contingencyPlan && (
                          <div className="bg-stone-50 border border-stone-200 p-2">
                            <div className="text-[10px] font-mono text-stone-400 mb-1">兜底预案</div>
                            <div className="text-stone-700 whitespace-pre-wrap">{risk.contingencyPlan}</div>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {!closed && (
                        <button
                          ...
                          className="inline-flex h-8 w-8 items-center justify-center border border-stone-300 text-stone-500 hover:bg-stone-50 disabled:opacity-50"
                          title="关闭风险"
                        >
...
function RiskMetric({ label, value, accent }: { label: string; value: number; accent?: string }) {
  return (
    <div className="ce-card p-3">
      ...
      <div className={`mt-1 text-2xl font-semibold ${accent ?? 'text-stone-800'}`}>{value}</div>
    </div>
  );
}
...
function RiskTextarea({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="block">
      <span className="block text-[10px] font-mono uppercase tracking-wider text-stone-400 mb-1">{label}</span>
      <textarea
        value={value}
        onChange={(event) => onChange(event.target.value)}
        rows={3}
        className="w-full border border-stone-200 px-3 py-2 text-sm outline-none resize-none focus:border-stone-400"
      />
    </label>
  );
}
