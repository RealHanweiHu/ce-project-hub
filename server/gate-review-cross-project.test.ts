...
import { activityLogs, projectGateReviews, projects } from "../drizzle/schema";
...
describe("gate review 跨项目 IDOR 防护", () => {
  ...
  it("B 项目 owner 正常更新/删除自己项目的评审", async () => {
    const caller = appRouter.createCaller(makeCtx(OWNER_B));
    await caller.gateReviews.update({ id: reviewInB, projectId: PROJECT_B, decision: "conditional" });
    const db = await getDb();
    const [row] = await db!.select().from(projectGateReviews).where(eq(projectGateReviews.id, reviewInB));
    expect(row.decision).toBe("conditional");

    await caller.gateReviews.delete({ id: reviewInB, projectId: PROJECT_B });
    const rows = await db!.select().from(projectGateReviews).where(eq(projectGateReviews.id, reviewInB));
