...
export async function createGroupChat(
  ...
  try {
    const resp = await fetchDingtalkApi((token) => ({
      url: `https://oapi.dingtalk.com/chat/create?access_token=${encodeURIComponent(token)}`,
      init: {
        method: "POST",
    ...
    if (!resp) return { ok: false, error: "获取钉钉 access_token 失败" };
...
export async function sendToGroupChat(chatId: string, title: string, markdown: string): Promise<boolean> {
  ...
  try {
    const resp = await fetchDingtalkApi((token) => ({
      url: `https://oapi.dingtalk.com/chat/send?access_token=${encodeURIComponent(token)}`,
      init: {
        method: "POST",
        headers: { "Content-Type": "application/json" },
