...
import { ENV } from "./env";
import { fetchDingtalkApi, isDingtalkConfigured, resolveDingtalkCorpUserId } from "./dingtalk";
...
export async function sendWorkNotification(corpUserIds: string[], title: string, markdown: string): Promise<void> {
  ...
  try {
    await fetchDingtalkApi((token) => ({
      url: `https://oapi.dingtalk.com/topapi/message/corpconversation/asyncsend_v2?access_token=${encodeURIComponent(token)}`,
      init: {
        method: "POST",
