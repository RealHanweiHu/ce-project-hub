 ...
 * 完成 gate 任务的唯一路径是 gateReviews.confirmAndAdvance。
 * 取消勾选保留直连（它是撤销手段，方向是上锁不是解锁，且 UI 有意暴露）。
 */
...
describe("gate task guard on tasks.setCompleted", () => {
  ...
  it("gate 任务取消勾选仍走直连（撤销手段，方向是上锁）", async () => {
    const db = await getDb();
    ...
    const caller = tasksRouter.createCaller(makeCtx(OWNER));
    await caller.setCompleted({ projectId: PROJECT, phaseId: "concept", taskId: "c6", completed: false });
    const task = (await getProjectTasks(PROJECT, "concept")).find((t) => t.taskId === "c6");
