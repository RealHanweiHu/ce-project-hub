...
import { buildActionCardExecutePath, toAbsoluteAppUrl } from "../shared/action-links";
...
async function buildActionItemButtons(input: ActionItemNotifyInput, actionItemId?: number | null): Promise<WorkNotificationButton[]> {
  ...
  if (actionItemId && (input.level ?? "owner") === "owner") {
    buttons.push(await executeButton("明早处理", {
...
export function actionDedupeKey(input: {
  ...
  return `${input.kind}:${input.entityId}:${input.recipientUserId}:${input.level ?? "owner"}`;
}
