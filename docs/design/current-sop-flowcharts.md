...
> ⚠ **本文档仅覆盖 NPD/ECO/IDR，已不完整。** 最新权威设计（含 JDM 客户委托、
> OBT 转产导入两条赛道及客户签核/电池安全等强制交付物）见
> [`2026-06-29-sop-design.md`](./2026-06-29-sop-design.md)；一切以代码 `shared/sop-templates.ts` 为准。
>
> 来源：`shared/sop-templates.ts` 当前模板定义。  
> 范围：NPD 新产品开发、ECO 迭代升级、IDR 外观翻新（JDM/OBT 见上方最新文档）。  
...
```mermaid
flowchart LR
  N0["开始<br/>新产品开发项目"] --> N1["P1 概念阶段<br/>2-4周<br/>Gate: 立项评审 / Project Charter<br/>交付物: 市场调研、概念书、商业可行性、立项申请、认证路线图初判"]
  N1 --> N2["P2 规划阶段<br/>3-4周<br/>Gate: Kickoff评审<br/>交付物: PRD、PSD、甘特图、BOM v0.1、认证路线图、电芯复用/定点与二供策略"]
  N2 --> N3["P3 设计阶段<br/>6-12周<br/>Gate: 设计冻结评审<br/>交付物: ID/MD/PCB/SW/BOM、安全FMEA、电芯厂审核或复用确认、保护电路评审或复用确认"]
  N3 --> N4["P4 EVT 工程验证<br/>4-6周<br/>Gate: EVT评审<br/>交付物: EVT样机、功能测试报告、问题清单、PCB v2"]
  N4 --> N5["P5 DVT 设计验证<br/>4-8周<br/>Gate: DVT评审<br/>交付物: DVT样机、可靠性、认证、模具T1、PFMEA/CTQ"]
  N5 --> N6["P6 PVT 试产验证<br/>3-6周<br/>Gate: MP准备就绪评审<br/>交付物: 试产、SOP/WI、良率、治具、EOL 100%、UN38.3/MSDS/电池安全证或复用确认"]
...
flowchart TB
  subgraph N1["P1 概念阶段"]
    ...
    C3 --> C4["技术可行性评估/认证初判"] --> C5["商业可行性分析"] --> C6["立项评审 (Gate 1)"]
  end

  subgraph N2["P2 规划阶段"]
    P1["产品需求文档 (PRD)"] --> P2["产品规格书 (PSD)"] --> P3["项目时程规划"]
    P3 --> P4["BOM初版"] --> P5["关键供应商初选"] --> P5A["电芯复用/定点与二供策略"]
    P5A --> P6A["认证路线图"] --> P6["团队组建与资源分配"] --> P7["Kickoff会议"]
  end

  subgraph N3["P3 设计阶段"]
    D1["ID 工业设计"] --> D2["MD 结构设计"] --> D3["EE 电子原理设计/保护输入"] --> D4["PCB Layout"]
    D4 --> D5["SW 软件架构"] --> D6["DFM/DFA 评审"] --> D6A["安全FMEA与危害分析"]
    D6A --> D7["关键料件定型"] --> D7A["电芯厂审核/复用确认"] --> D7B["保护电路评审/复用确认"] --> D8["设计冻结评审 (Gate 3)"]
  end
  ...
  subgraph N5["P5 DVT 设计验证"]
    ...
    V4 --> V5["软件功能完整测试"] --> V6["包装设计验证"] --> V7["量产工艺评估/PFMEA"] --> V8["DVT评审 (Gate 5)"]
  end

  subgraph N6["P6 PVT 试产验证"]
    PV1["试产规划"] --> PV2["SOP/WI制定"] --> PV3["治具与测试程序/EOL 100%"] --> PV4["试产 (50-300台)"]
    PV4 --> PV5["良率分析与改善"] --> PV6["包装与物流验证"] --> PV7["品质标准固化/安全性能限值"] --> PV8["PVT评审 (Gate 6)"]
  end
