...
> 六个轨道统一遵循：发布 Gate 只生成量产 Revision 并进入稳定期；Close Gate 才关闭归档项目。发布后的长期 ECO、售后、OTA、CIP 和 EOL 治理归产品轴，不作为项目关闭阶段的常驻任务。
...
flowchart LR
  ...
  Rev --> Stable["2–8 周爬坡/稳定期<br/>两期 QA 确认记录 · ≥14 天 · 无 P0/P1"]
  ...
  class R1,R2,R3,R4,R5,R6,HardGate,Rev,Close rel;
```
...
### 10.2 发布时 4 道绝对硬卡（`releaseProject`）
